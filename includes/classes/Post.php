...
class Post { //Object
	...
	// Submits the post and inserts it into the database
	public function submitPost($body, $user_to, $imageName) {
		...
		if($check_empty != "" || $imageName != "") {
			...
			//If the user is on their own profile, user_to is 'none'.
			if($user_to == $added_by) {
				$user_to = "none";
			}
			...
			// Stop words to not count for the trending words.
			$stopWords = "a about above across after again against all almost alone along already
			 ...
			 grouping groups guy guys h had has have having he her here herself high high high higher
             ...
             //Converts stop words into array - split at white space.
			$stopWords = preg_split("/[\s,]+/", $stopWords);

			//Removes all punctionation.
			$no_punctuation = preg_replace("/[^a-zA-Z 0-9]+/", "", $body);

			//Predicts whether user is posting a url. If so, do not check for trending words.
			if(strpos($no_punctuation, "height") === false && strpos($no_punctuation, "width") === false
				...
				//Converts users post (with punctuation removed) into array - split at white space.
		...
		if(mysqli_num_rows($data_query) > 0) {
			...
			while($row = mysqli_fetch_array($data_query)) {
				...
				$user_logged_obj = new User($this->con, $userLoggedIn);
					...
					//Once 10 posts have been loaded, break the load.
					if($count > $limit) {
						break;
					}
					else {
					...
					$str .= "<div class='status_post'>
								...
								<div class='post_options' onClick='javascript:toggle$id()'>
	...
	public function loadPostsFriends($data, $limit) {
		...
		if(mysqli_num_rows($data_query) > 0) {
			...
			while($row = mysqli_fetch_array($data_query)) {
				...
				<?php
				} //End of if($user_logged_obj->isFriend($added_by))
			} //End while loop
		...
		if(mysqli_num_rows($data_query) > 0) {
			...
			while($row = mysqli_fetch_array($data_query)) {
				...
				if($row['user_to'] == "none") {
					...
					$user_details_query = mysqli_query($this->con, "SELECT profile_pic, username FROM users WHERE username='$added_by'");
					$user_row = mysqli_fetch_array($user_details_query);
					$profile_pic = $user_row['profile_pic'];
					$username = $user_row['username'];
